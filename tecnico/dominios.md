...
| `calendar` | Visão de leitura das provas com janela agendada (`/app/calendario`); gate de assinante. Ver [tecnico/calendario.md](calendario.md). |
| `exam-notification` | Outbox Mongo (sem Redis) dos e-mails de "atividade disponível" quando a janela abre; cron + reenvio manual. Ver [tecnico/calendario.md](calendario.md). |
...
| `library` | **Biblioteca** — upload de arquivos (presigned S3/Railway Buckets), extração de texto, acesso por dono/org/assinante, taxonomia por disciplina+segmento; fonte de conteúdo do `ai-ops`. Ver [tecnico/biblioteca.md](biblioteca.md). |
...
| `billing` | Créditos, ledger, débito atômico, assinaturas Stripe, top-ups por cartão, webhook, modos de billing de organização. PIX (AbacatePay) **desativado** por kill-switch — ver [tecnico/billing-ledger.md](billing-ledger.md) e [tecnico/integracoes.md](integracoes.md). |
...
- **Exam** — id, classId, courseId (snapshot), ownerId, title, description (≤ **10.000** chars),
  **activityType** (`exam|mockExam|quiz|exerciseList`, default `exam`, puramente classificatório —
  ...
  dedicado) — ver [tecnico/ai-ops.md](ai-ops.md) e [tecnico/calendario.md](calendario.md).
...
Detalhe de créditos em [tecnico/billing-ledger.md](billing-ledger.md); de geração/correção em [tecnico/ai-ops.md](ai-ops.md).
